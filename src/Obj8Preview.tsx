...
const HIGHLIGHT_STORAGE_KEY = "xplane2flt-preview-highlight-color";
...
export function Obj8Preview({ sourceFile, models, onToggleModel }: Obj8PreviewProps) {
  ...
  const highlightMaterialsRef = useRef<THREE.MeshBasicMaterial[]>([]);
  const highlightColorRef = useRef<string>(DEFAULT_HIGHLIGHT_COLOR);
  ...
  const [highlightColor, setHighlightColor] = useState(() => {
    ...
    const saved = window.localStorage.getItem(HIGHLIGHT_STORAGE_KEY);
    return HIGHLIGHT_COLORS.some((color) => color.value === saved) ? saved! : DEFAULT_HIGHLIGHT_COLOR;
  });
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(HIGHLIGHT_STORAGE_KEY, highlightColor);
    for (const material of highlightMaterialsRef.current) material.color.set(highlightColor);
  }, [highlightColor]);
  ...
  useEffect(() => {
    ...
    highlightRoot.name = "Selection highlight";
    ...
    const clearHighlight = () => {
      highlightRoot.clear();
      for (const material of highlightMaterialsRef.current) material.dispose();
      highlightMaterialsRef.current = [];
    };
    const showHighlight = (modelPath: string) => {
      clearHighlight();
      const modelGroup = root.children.find((child) => child.userData.modelPath === modelPath);
      ...
      modelGroup.traverse((child) => {
        ...
        highlightMaterialsRef.current.push(material);
        highlightRoot.add(overlay);
      });
    };
    const onPointerUp = (event: PointerEvent) => {
      ...
      const modelPath = hit?.object.userData.modelPath as string | undefined;
      const modelName = hit?.object.userData.modelName as string | undefined;
      if (modelPath && modelName) {
        setSelectedModel({ name: modelName, path: modelPath });
        showHighlight(modelPath);
      } else {
        setSelectedModel(null);
        clearHighlight();
      }
    ...
    <section className="panel preview-panel">
      <div className="panel__heading preview-panel__heading">
        ...
        <div className="selection-actions">
          ...
          <div className="highlight-picker">
            <button
              className="highlight-picker__toggle"
              type="button"
              aria-expanded={paletteOpen}
              aria-haspopup="listbox"
              onClick={() => setPaletteOpen((open) => !open)}
            >
              <span className="highlight-picker__active" style={{ backgroundColor: highlightColor }} aria-hidden="true" />
              Highlight
            </button>
            {paletteOpen && (
              <div className="highlight-palette" role="listbox" aria-label="Selection highlight color">
                {HIGHLIGHT_COLORS.map((color) => (
                  <button
                    key={color.value}
                    className={highlightColor === color.value ? "is-active" : ""}
                    type="button"
                    role="option"
                    aria-selected={highlightColor === color.value}
      ...
      {selectedModel && (
        <div className="preview-selection">
          <span><strong>{selectedModel.name}</strong> selected in the viewport</span>
          <button type="button" onClick={() => {
            onToggleModel(selectedModel.path);
