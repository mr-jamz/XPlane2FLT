...
const supported = new Set([1, 2, 4, 5, 10, 11, 64, 67, 72]);
let offset = 0; let record = 0; let currentParent = 0; let headerOpen = false;
let groups = 0; let objects = 0; let faces = 0; let vertices = 0;
const stack = [];
while (offset + 4 <= bytes.byteLength) {
  ...
  if (opcode === 10) {
    ...
    if (opcode === 2) groups += 1; if (opcode === 4) objects += 1; if (opcode === 5) faces += 1;
...
console.log(JSON.stringify({ mcxReaderVerified: true, records: record, groups, objects, faces, vertices }, null, 2));
