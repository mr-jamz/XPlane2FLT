...
import { HIGHLIGHT_COLORS, previewTriangleIndices, selectTriangleIndices } from "../src/Obj8Preview";
...
describe("OBJ8 preview geometry", () => {
  it("offers a compact set of unique highlight colors", () => {
    expect(HIGHLIGHT_COLORS.map((color) => color.name)).toEqual([
      "Red", "Orange", "Yellow", "Green", "Blue", "Purple", "White",
    ]);
    expect(new Set(HIGHLIGHT_COLORS.map((color) => color.value)).size).toBe(HIGHLIGHT_COLORS.length);
  });
